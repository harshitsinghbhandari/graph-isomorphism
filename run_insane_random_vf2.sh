...
# Launch four fresh random-pair VF2-vs-ours comparison shards in parallel.
#
# Default workload:
#   shard_08_12 : n = 8,9,10,11,12   with 250 random pairs per n
#   shard_15_20 : n = 15,20          with 250 random pairs per n
#   shard_25    : n = 25             with 250 random pairs
#   shard_30    : n = 30             with 250 random pairs

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "${ROOT_DIR}"

OUT_DIR="${1:-insane_random_vf2}"
PAIRS="${PAIRS:-250}"
...
start_shard() {
  ...
  rm -rf "${shard_dir}"
  mkdir -p "${shard_dir}"
  nohup "${PYTHON_BIN}" random_vf2_comparison.py \
    --sizes "${sizes}" \
    --pairs "${PAIRS}" \
    --seed "${seed}" \
...
echo "Launching random VF2-vs-ours comparison"
...
start_shard "shard_08_12" "8,9,10,11,12" "90812"
start_shard "shard_15_20" "15,20" "91520"
start_shard "shard_25" "25" "925"
start_shard "shard_30" "30" "930"
...
Aggregate after/during run:
  ${PYTHON_BIN} aggregate_random_vf2.py --root ${OUT_DIR}
