...
The central result of Phase 1 is intentionally narrow: the relaxation gives an
interpretable soft correspondence matrix, the degree-profile term improves
separation over an adjacency-only objective, and the current bottleneck is now
clearly located in rounding quality and solver scaling.
\end{abstract}
...
This index is useful for plots and experiment summaries.
It is not a proof.
...
Hungarian rounding remains an important internal rounding baseline.
...
The presentation branch keeps the experiment workflow focused on our method.
The relevant entry point is \texttt{main.py}.

\subsection{Single Isomorphic Case}

The command
\begin{quote}
\texttt{python main.py single --n 101}
\end{quote}
generates one clearly isomorphic pair, solves the relaxation, rounds the
solution, checks $AP^*=P^*B$, and writes a matrix viewer.
This is the best demonstration path because the expected positive certificate
is known from construction.

\subsection{Small Range Experiment}

The command
\begin{quote}
\texttt{python main.py experiment --n-min 5 --n-max 30 --pairs 5}
\end{quote}
runs the current solver on isomorphic and independently sampled random pairs.
It records $Z^*$, $I$, runtime, certificate counts, an HTML report, and a
matrix viewer.

The random-pair label is an experiment label, not a proof.
For moderate and large $n$, independently sampled dense graphs are almost
surely non-isomorphic, but the solver's only formal positive statement remains
the rounded certificate check.

\subsection{Observed Runtime Pattern}

The QP solver dominates runtime and memory.
A practical observation from the server run was that around $n=50$, the current
QP path was already taking about 11--12 seconds per random pair.
...
The report should therefore treat them as random-pair experiments, not formal
non-isomorphism certificates.
...
\begin{itemize}[leftmargin=*]
    ...
    \item Hungarian-vs-hyperplane rounding study on the same saved $X^*$;
...
The current project should be presented as a research prototype.
Its value is that it makes graph isomorphism visible as an optimization
problem: the soft matrix $X^*$, the objective components, and the rounded
certificate all expose where the method succeeds or fails.
