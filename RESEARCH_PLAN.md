...
This branch is for going beyond the presentation version. The goal is to turn
the current relaxation-and-rounding prototype into a serious experimental
research codebase.
...
- Separate model construction time, solver time, rounding time, and verification
...
- mean;
- standard deviation;
- confidence intervals when enough samples exist;
- sample count;
- failure count;
- timeout count.
...
- certification rate vs `n`;
- confusion matrix by `n`;
- score separation violin/box plots;
- runtime vs `n`;
- objective component scaling;
- `X*` entropy vs success/failure.

Avoid overclaiming:

- Do not generalize dense-random success to all graphs.
- Do not claim non-isomorphism from failed rounding.
- Do not claim a fixed polynomial QP solve bound unless citing and mapping a
specific theorem.

## Phase 7: Paper-Quality Narrative

The strongest possible final story:

1. GI can be expressed as `AP = PB`.
2. Relaxing `P` to the Birkhoff polytope gives an interpretable soft matching.
3. The adjacency-only relaxation is too weak on many cases.
4. Degree/WL/higher-order features sharpen `X*`.
5. Hyperplane rounding is a geometry-aware extraction method inspired by
Max-Cut.
6. Exact certification remains `AP = PB`.
7. The method is not a replacement for exact GI solvers, but it exposes a useful
optimization view and failure diagnostics.
